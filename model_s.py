...
if __name__ == "__main__":
    ...
    parser.add_argument('--topK', type=int, default='10', help='number of similarity users or items')
    parser.add_argument('--alph', type=float, default='0.5', help='alph')
    parser.add_argument('--time_len', type=int, default='20', help='time_len')
    parser.add_argument('--sprasity', type=str, default='0.3', help='sprasity')
    args = parser.parse_args()
    #参数
    topK = args.topK
    alph = args.alph
    time_len=args.time_len
    sprasity = args.sprasity
    #读取('../../data/rt.npy')
    data = np.load(f'./data/rt_sprasity{sprasity}.npy', allow_pickle=True)
    train_qos = data[:time_len-1,:,:]
    test_qos = data[time_len-1,:,:]
    print(f'setting:{topK}_{alph}_{time_len}_{sprasity}')
    print('upcc模型训练')
    #进行upcc模型训练
    ...
    #将上面数据保存到mat中
    scio.savemat(f'./data/result_{topK}_{alph}_{time_len}_{sprasity}.mat',{
        'topK':topK,
        'alph':alph,
        'time_len':time_len,
        'sprasity':sprasity,
